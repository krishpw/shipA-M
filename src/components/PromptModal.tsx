...
export const PromptModal: React.FC<PromptModalProps> = ({ isOpen, mode, onClose, onSubmit }) => {
  ...
  const themeColor = isCreate ? 'rose' : 'slate';
  const themeBg = isCreate ? 'bg-[#500000]' : 'bg-slate-700';
  const themeHover = isCreate ? 'hover:bg-[#3A0000]' : 'hover:bg-slate-800';
  const themeLight = isCreate ? 'bg-rose-50' : 'bg-slate-100';
  const themeText = isCreate ? 'text-[#500000]' : 'text-slate-700';

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 font-sans">
      <div className={`bg-white rounded-3xl shadow-2xl w-full max-w-lg flex flex-col border-4 ${isCreate ? 'border-rose-100' : 'border-slate-100'} animate-in fade-in zoom-in duration-200 scale-95 sm:scale-100 overflow-hidden`}>
        
        {/* Header */}
        <div className={`flex items-center justify-between p-6 border-b ${isCreate ? 'border-rose-50 bg-gradient-to-r from-rose-50 to-red-50' : 'border-slate-50 bg-gradient-to-r from-slate-50 to-gray-50'}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
                ...
                <p className={`text-xs font-bold uppercase tracking-wide ${isCreate ? 'text-[#500000]' : 'text-slate-500'}`}>
                    POWERED BY GEMINI 3
                </p>
            </div>
          </div>
        ...
        <div className="p-6 bg-white">
          ...
          <form onSubmit={handleSubmit}>
            <textarea 
              ...
              disabled={isLoading}
              className={`w-full h-32 resize-none bg-slate-50 border-2 border-slate-200 rounded-xl p-4 font-medium text-slate-700 focus:outline-none focus:ring-4 transition-all placeholder:text-slate-400 mb-4 ${isCreate ? 'focus:border-[#500000] focus:ring-rose-100' : 'focus:border-slate-400 focus:ring-slate-100'}`}
              autoFocus
            />
